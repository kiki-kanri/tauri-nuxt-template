...
android_download_file() {
    ...
    if [[ -s "${output}" ]]; then
        log_success "download exists: ${output}"
        return 0
    fi
...
android_install_cmdline_tools() {
    ...
    if android_cmdline_tools_are_valid; then
        log_success "Android command-line tools exist: ${SDK_ROOT}/cmdline-tools/latest"
        return 0
    fi

    if [[ -e "${SDK_ROOT}/cmdline-tools/latest" ]]; then
        log_warn "Android command-line tools are incomplete or invalid; reinstalling: ${SDK_ROOT}/cmdline-tools/latest"
    ...
    copy_dir_contents "${extracted}/cmdline-tools" "${SDK_ROOT}/cmdline-tools/latest"
...
android_cleanup_duplicate_cmdline_tools() {
    ...
    for duplicate in "${SDK_ROOT}"/cmdline-tools/latest-*; do
        ...
        log_warn "Removing duplicate Android command-line tools directory: ${duplicate}"
...
android_cmdline_tools_are_valid() {
    ...
    android_with_java "${SDK_ROOT}/cmdline-tools/latest/bin/sdkmanager" --version >/dev/null 2>&1
}

android_sdkmanager() {
    android_with_java "${SDK_ROOT}/cmdline-tools/latest/bin/sdkmanager" --sdk_root="${SDK_ROOT}" "$@"
}

android_with_java() {
    ANDROID_USER_HOME="${ANDROID_USER_HOME}" JAVA_HOME="${JDK_ROOT}" PATH="${JDK_ROOT}/bin:${PATH}" GRADLE_USER_HOME="${GRADLE_USER_HOME}" "$@"
}
...
android_install_sdk_packages() {
    ...
    log_info "==> Resolving latest stable Android SDK packages"
    ...
    log_info "platform: ${platform_pkg}"
    log_info "build-tools: ${build_tools_pkg}"
    log_info "ndk: ${ndk_pkg}"
...
android_create_signing_material() {
    ...
    if [[ ! -f "${keystore}" ]]; then
        log_info "==> Creating local Android signing keystore"
        ...
        log_success "signing keystore exists: ${keystore}"
    fi

    write_file_if_changed "${properties}" <<EOF_PROPERTIES
...
android_write_env_file() {
    ...
    write_file_if_changed "${ENV_FILE}" <<EOF_ENV
