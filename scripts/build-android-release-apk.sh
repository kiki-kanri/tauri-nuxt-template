...
ensure_android_project() {
    if [[ -d "$ANDROID_DIR" ]]; then
        ok "Tauri Android project exists: $ANDROID_DIR"
        return 0
    fi

    step "Initializing Tauri Android project"
    (cd "$APP_DIR" && cargo tauri android init --ci --skip-targets-install)
}

build_android_release() {
    step "Building Android release APK and AAB"
...
copy_release_outputs() {
    ...
    step "Copying Android release outputs to dist/android"
    safe_rm_rf_under "$ANDROID_DIST_ROOT" "$ROOT"
    ensure_dir "$ANDROID_DIST_ROOT"

    while IFS= read -r output; do
        copy_path_into_dir "$output" "$ANDROID_DIST_ROOT"
        copied=$((copied + 1))
        ok "Android output: $ANDROID_DIST_ROOT/$(basename "$output")"
